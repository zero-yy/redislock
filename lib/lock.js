 ...
 *                         first attempt failed (default: 0, infinite: -1)
...
function Lock(client, options) {
  ...
  for (key in Lock._defaults) {
    ...
    if (key in options) {
      this[key] = parseInt(options[key], 10) || 0;
    }
  }
}
...
Lock.prototype._setupLuaScripts = function(client) {
  ...
  if (!Lock._shavaluators[id]) {
    ...
    Lock._shavaluators[id].add('setnxex', scripts.setnxex);
  }
...
Lock.prototype._attemptLock = function(key, retries) {
  ...
  return shaval.execAsync('setnxex', [key], [this._id, ttl]).then(function(res) {  
    if (!res && retries === 0) {
    ...
    return Promise.delay(lock.delay).then(function() {
      ...
      return lock._attemptLock(key, retries);
    });
  });
};
